...
**Key API:** Bean `GenericTmfClient` (sync; `-rest`) / `GenericReactiveTmfClient` (`-reactive`); typed clients via `TmfClientFactory.create(serverConfig, "endpoint", R.class)`; pub/sub `TmfHubClient`; pagination `TmfOffsetRequest`/`TmfPage`; errors `OpenTmfClientResponseException`/`OpenTmfClientNotFoundException`. Since **3.0.0: ENTITY VIEW** — `client.entity()` returns a `TmfEntityClient` (sync) / `ReactiveTmfEntityClient` (reactive) whose verbs answer `ResponseEntity<...>` / `Mono<ResponseEntity<...>>`, so the status code and response headers (`Location` after a create, `ETag` for a conditional follow-up, custom `X-*`) are readable on the **success** path; same eight verb families and 8-overload ladder as the body view, and `sub(...)` composes (`client.sub(...).entity()`). Since 2.1.0: **`sub(template, vars...)`** on both `TmfClient` and `ReactiveTmfClient` returns a derived client scoped to a nested path, so every verb (get/list/listAll/listPaged/post/put/patch/delete) works against it unchanged and at arbitrary depth — e.g. `orderClient.sub("/{orderId}/action/{action}/item", orderId, action).get(itemId, Item.class)` issues `GET /order/{orderId}/action/{action}/item/{itemId}`; vars bind as URI template variables and are strictly encoded (a value containing `/`, `:` or `{` cannot inject), and template/argument arity is validated before any request is issued. **Bean name = `<server>.<endpoint>TmfClient`** (relaxed-bound keys, e.g. qualifier `catalogManagement.productOfferingTmfClient`).
...
```
**Gotchas:** Unlike the raw http-clients `RestClient`, `GenericTmfClient` **auto-attaches** the bearer token (via `HeaderUtil` + the `<clientRef>TokenService`) and validates it's present — so reach for api-clients (not a hand-used `RestClient`) whenever you're calling a TMF resource and want auth handled. Endpoints whose path ends `/hub` get only a `TmfHubClient`, no generic bean. REST and reactive are separate modules — don't pull WebFlux into a servlet app. Endpoint `fixed-headers` override server-level on collision. Since 2.1.0, direct callers of `UriBuilderUtil.buildUriWithId(server, endpoint, id, null)` passing a **null literal** must cast it to `(TmfRequestContext) null` — the new `SubResourcePath` overload makes the call ambiguous otherwise; binary compatibility and the typed clients' API are unaffected. ⚠ **3.0.0 upgrade:** reactive `listPaged*` now returns `Mono<TmfPage<List<R>>>`, not `Mono<TmfPage<Flux<R>>>` — the old body `Flux` was a single-subscription live connection stream, so reading only the page metadata left the connection undrained and the content could never be read twice. Migrate `flatMapMany(TmfPage::getContent)` → `flatMapIterable(TmfPage::getContent)` (the repo ships a MIGRATION.md). Reactive `list`/`listAll` keep their `Flux<R>` signatures but now buffer each page before emitting its items. **`AuthType.NONE` finally works** — such a client used to throw `IllegalArgumentException: Authorization token must not be empty.` on every call before anything reached the network, and now sends no `Authorization` header at all; a token passed to a `…WithToken` overload on a NONE client goes out verbatim as the whole credential, so include the scheme yourself (`"Bearer eyJ…"`). Mirror-image change for authenticated clients: a BEARER/BASIC client whose token service returns a **blank** token now fails locally instead of sending `Authorization: Bearer ` and collecting a remote 401 — the guard was always documented but never actually fired. No entity form exists for `listAll*` (N pages means N header sets) or `listPaged*` (`TmfPage` already *is* the header-derived view). Errors still throw `OpenTmfClientResponseException`, whose headers remain the source for a failed response. Extension point moved: `HeaderUtil.headersConsumer(...)` takes a fifth `AuthType` argument and owns the whole authorization decision, `validateAuthorization` is gone, and no four-argument overload was kept — every call site must state its auth type.
...
**Key API:** Per-`<id>` beans `<id>RestClient`, `<id>RestTemplate`, `<id>WebClient`, `<id>TokenService`. Utils `SyncClientUtil` (`executeWithRetry`, `emptyOn404`), `WebClientUtil`, `HttpClientUtil`. Ordering marker bean `opentmfHttpClientsStarter` (`@DependsOn`). Since **2.2.0: TOKEN-MINT RETRY + OBSERVABILITY** — a bearer token mint (sync and reactive) is retried exactly once, immediately, on a transport-level failure (stale keep-alive, reset, premature EOF, I/O timeout at headers or body stage), never on a status error or an open circuit breaker; a mint that still fails throws **`BearerTokenTransportException`** (`org.opentmf.client.bearer.exception`: `tokenUrl`, `attempts`, cause) instead of the raw `ResourceAccessException`/reactor error — not a response exception, so the retry utilities never retry it (map it to 503). One `INFO` per successful mint (issuer URL, scope, attempt, duration) and, with a `MeterRegistry`, counter `opentmf.client.token.fetch{client, outcome=ok|retried|failed}` (Prometheus `opentmf_client_token_fetch_total`); `TokenFetchListener` (`org.opentmf.client.bearer.observe`) / `TokenFetchMeters` bean for hand-wired token clients; `RestClientRegistrar.createTokenService(clientId, RestClient, ClientProperties)` overload. Since 2.1.4: **`HttpClientRegistry`** bean for runtime/programmatic clients (`getOrCreate(name, ClientType, props)` / `replace` / `evict`, reactive `getOrCreateReactive`; handle exposes `restClient()`/`webClient()`/`tokenService()`); **`ClientPropertiesValidator.validate(ClientType, props)`** — static, side-effect-free, returns ALL findings at once (SRE "test connection" checklists); **`OpenTmfClientResilienceException`** for circuit-open/bulkhead-full rejections (deliberately OUTSIDE the `OpenTmfClientResponseException` hierarchy so retry utils never retry it; map to 503).
...
**Key API:** `@QuerydslPredicate(root = Entity.class) Predicate` param (repo extends `QuerydslPredicateExecutor`); `@Tmf630Response` for status + `Content-Range`/`X-Total-Count` headers + field selection; `Tmf630Util.tmfPage(Page)`; enum `TmfOperator`. Since **3.4.0: REGEX ON JPA = LIKE SUBSET** — with `regex.enabled=true`, `.regex`/`.regexi`/Part 6 `=~` on an `@Entity` root render literals, `\`-escapes, `.`, `.*`, `.*?`, edge `^`/`$` and the `i` flag as `like`/`lower(…) like lower(…)` (unanchored = contains), same rows as Mongo/JSONB; anything else is a 400 naming the subset. JSONB `filter=` `=~` lowers to `like_regex`. Since **3.3.0: BOUNDED `Link` HEADER + QUERY-PARAMETER GUARD** — the paging `Link` header is **omitted** when any single query-parameter value exceeds `paging.link.max-param-value-length` (256) or the assembled header would exceed `paging.link.max-length` (2048); `X-Total-Count`/`X-Result-Count`/`Content-Range` and the status are unchanged (new `Tmf630LinkHeaderSettings` record + `Tmf630Util.applyLinkHeader(...)` overload). `Tmf630QueryLimitsInterceptor` (on every mapping, via `Tmf630QueryLimitsAutoConfiguration`) rejects a single value longer than `query-limits.max-param-value-length` (2048) with **400** and a query string longer than `query-limits.max-query-string-length` (4096) with **414**, both as TMF `ErrorMessage`; `Tmf630HeadersTooLargeRecoveryResolver` answers Tomcat's `HeadersTooLargeException` once with a chunked 500 TMF body instead of an empty `500` + `Connection: close`. Since **3.2.0: PASS-THROUGH PARAMETERS** — `@Tmf630PassThrough({"version"})` on the handler (or its API-interface method) exempts exact-named non-entity query parameters from both filter terminals (`@QuerydslPredicate`, `@Tmf630JsonbFilter`) under `on-unknown-field=REJECT`; exact names only (`version.eq` is still a filter key), nothing global. Since 3.0.0 (major), optional modules: `tmf630-toolkit-jpa-correlated-sort` (`Tmf630JpaCorrelatedSortExecutor` — dotted/`[k=v]`-correlated/multi-hop sorts, `min()`/`max()` wrappers, auto-wired on an `EntityManager`), `tmf630-toolkit-jsonb` (Postgres `payload jsonb` backend: `@Tmf630JsonbBacked`, filter/write executors, sub-resource controller — the escape hatch for everything JPA rejects), `tmf630-toolkit-mongo-split-collection` (`@Tmf630MongoSplitBacked`, split-aware translators); plus TMF-630 Part-4 **entity versioning**: `TmfVersionedId` path binding (`/X:(version=1.0)`), `@Tmf630Versioned(versionOrder = LEX|NUMERIC_STRING|SEMVER)`, `Tmf630VersionResolver` (JPA/Mongo/Jsonb impls).
...
**Config:** `opentmf.tmf630.paging.*` (`default-limit=50`, `max-limit=500`, `strict-mode=true`, `link.max-param-value-length=256`, `link.max-length=2048`); `opentmf.tmf630.query-limits.*` (`enabled=true`, `max-param-value-length=2048`, `max-query-string-length=4096` — independent of `paging.enabled`); `…field-selection.default-depth=1`; `…attribute-filtering.*` (`implicit-eq-enabled=true`, `on-unknown-field=REJECT`, `regex.enabled=false`, `json-path-filter.enabled=true`). Autoconfigs `Tmf630WebMvcConfigurer`, `Tmf630AttributeFilteringAutoConfiguration`, `Tmf630FieldSelectionAutoConfiguration`.
**Snippet:**
```java
...
```
**Gotchas:** ⚠ **3.3.0 upgrade — `Link` is now optional on the wire:** any client that pages by following `Link: rel="next"` must fall back to `offset`/`limit` against `X-Total-Count` when the header is absent (it is omitted, never truncated, over the budget — TMF-630 Part 1 §4.5.1 makes it a SHOULD). A request carrying a query-parameter value > 2048 chars now answers 400 and a query string > 4096 chars answers 414 before any handler runs (`query-limits.enabled=false` switches the guard off); before 3.3.0 the same request on a paged endpoint was an empty `500` + `Connection: close`, because the `Link` header echoed the full query string four times past Tomcat's 8 KB header buffer. Use `tmf630-toolkit-all` to get both filtering autoconfigs. Unknown field/operator → 400 by default (`on-unknown-field=REJECT`). `default-depth=1`: `fields=address` expands only scalar sub-fields — use a dot-path or raise depth for associations. **3.0.0 breaking:** `.regex`/`.regexi` (and JSONPath `=~`) on a JPA `@Entity` now 400 at parse time instead of silently degrading to SQL `LIKE`; deprecated escape hatch `opentmf.tmf630.attribute-filtering.regex.allow-jpa-like-semantics=true` restores old behavior with a one-time WARN (jsonb module supports real regex). ⚠ **3.4.0 upgrade:** nothing changes unless `regex.enabled=true` (default `false`; a test pinning `400 "Regex operator is disabled."` stays green). Once enabled, in-subset patterns on a JPA root answer **200** where 3.0.0 – 3.3.0 refused them — re-pin any test that pinned that refusal in the same change that enables the operator. `allow-jpa-like-semantics` is now **inert** (binds, logs one WARN asking for removal; gone in 4.0.0), and the `500` it produced for `^`/`[...]`/`\d` patterns is a 400; caller-typed `%`/`_` now match literally on JPA instead of acting as wildcards. New `opentmf.tmf630.paging.nulls-last=true` decorates sorts with `nullsLast()` (synthetic `CASE WHEN` cost on non-native dialects). Sub-resource base controllers now carry `@Tmf630Response` themselves — subclasses get headers/`fields=` without annotating. JPA `filter=` got stronger: array correlation via correlated `EXISTS` on JOIN-mapped collections, positional `[N]` via `INDEX(alias)` (requires `@OrderColumn`); JSON-column collections still 400. Versioning `LEX` order trap: `"1.10" < "1.9"` — use `NUMERIC_STRING`/`SEMVER` (which sort in-JVM, O(N) per logical id). Mongo: OR-of-splits needs `$unionWith` — illegal inside a multi-document transaction. **3.0.1:** `.regex`/`.regexi` and the LIKE family now accept polymorphic (`Object`/`Serializable`) value fields on the **Mongo and JSONB** backends (TMF-620-style `productSpecCharacteristicValue.value`) — no effect on plain JPA entities. ⚠ **3.2.0 upgrade:** on a handler that binds a filter root (`@QuerydslPredicate(root = X)` / `@Tmf630JsonbFilter(root = X)`), a plain `sort=` key must be a declared field of `X`, so **a correct `sort=` on a getter-only (derived) property that Spring Data accepted now answers 400** — no typo needed; unknown or mistyped keys 400 too (were a 500 on JPA, a silent 200 on Mongo/JSONB). Audit which sort keys clients send before rollout. Opt out with your own `TmfSortKeyValidator` bean (`TmfSortKeyValidator.NONE`). Handlers without a filter root, and correlated `[k=v]`/JsonPath sort terms, are unchanged. Take **3.2.1 or later** rather than 3.2.0: it removes regex backtracking a crafted JSONB `filter=` could use to burn request-thread CPU.
...
**Key API:** Fully autoconfigured — set properties only. Types `OpenTmfSecurityProperties`, `SecureEndpoint`, `OtherEndpoints` enum (`ALLOW`/`DENY`/`AUTHENTICATED`). Since **3.3.0: JWKS HEALTH + METERS** — opt-in `opentmf.security.jwks.readiness: true` registers a `jwks` health contributor (one component per issuer: `UP`; `UP`+`stale: true` while a cached set still serves through failed refreshes; `DOWN` exactly when that issuer's bearer requests answer 503) that joins the `readiness` group under Kubernetes probes, and a NotReady probe triggers one paced background JWKS retry; always-on per-issuer meters when Micrometer is present: `opentmf.security.jwks.keys`, `opentmf.security.jwks.keys.age` (s), `opentmf.security.jwks.fetch.failures`; the boot line names the JWKS host and route (proxy/direct), host only. Since **3.2.0: CACHE-FIRST SIGNING KEYS** (`org.opentmf.security.jwks`) — per-issuer JWK set loaded by a background warm-up at startup, refreshed ahead of expiry, served stale through outages, refreshed on an unknown `kid` at most twice per interval; no request thread ever fetches after the first load. An issuer whose keys were never obtained answers a typed **503** + `Retry-After` + problem type `urn:opentmf:security:problem:signing-keys-unavailable` (rendered through the app's own error rendering; `JwtService.decodeJwt` throws `JwkSetUnavailableException`). Proxy honour: `issuers[].proxy` / `jwks.proxy` → JVM proxy props → `HTTPS_PROXY`/`HTTP_PROXY` + `NO_PROXY` env → direct. `TrustedIssuerKeys` (from `JwksAutoConfiguration`) is now the second ctor arg of `ServletJwtSupport`/`ReactiveJwtSupport`; the `*ResourceRetriever` classes are gone. Since **3.1.0: THE HTTP-STATUS MATRIX** — one fixed order, evaluated by the library before anything else, on every path (whitelisted included) and both ports: **404** no handler for the path (anonymous or authenticated) → **405, no `Allow`** path exists but the method is not implemented on it, unknown method names (`PROPFIND`, `BREW`) included → **401** no/invalid token → **403** valid token, roles insufficient. The 404/405 bodies are the application's own error rendering — the library raises the exceptions Spring raises natively (`NoHandlerFoundException` / `HttpRequestMethodNotSupportedException`; `ResponseStatusException` / `MethodNotAllowedException` on reactive) through the application's resolvers, so a `@ControllerAdvice`, `ProblemDetail` or TMF-`Error` renderer answers them like everything else. "The path is served" counts every `HandlerMapping` in dispatcher order (functional routes, resource handlers, consumer mappings). Not configurable: `unmatched-method-response` is removed and setting it fails startup. Removed types: `UnmatchedMethodResponse`, `MethodNotAllowedAccessDeniedHandler`, `MethodNotAllowedServerAccessDeniedHandler`, `BlacklistDecision`, `ReactiveBlacklistDenial`; `EndpointRules.allowedFor` → `EndpointRules.answerFor`. New: `ServletHttpStatusMatrixFilter` / `ReactiveHttpStatusMatrixFilter`, `RetiredPropertyGuard`. Since **3.0.0: HTTP-METHOD SEMANTICS** — a `GET` rule also covers `HEAD`; plain `OPTIONS` on a served path answers 200 + `Allow`; rule `method` values are restricted to upper-case `GET`/`POST`/`PUT`/`PATCH`/`DELETE`. Since **2.3.0: MULTI-ISSUER** — `opentmf.security.issuers[]` (each entry: `name`, `issuer`, `jwk-set-uri`, optional `audiences`, optional claim overrides); tokens route to the entry matching their `iss`, and `JwtDecoder`/`ReactiveJwtDecoder` route by issuer so `JwtService` and out-of-filter decoding keep working. Since 2.2.0: **custom 401/403 bodies via optional consumer beans** — servlet `AuthenticationEntryPoint` (401) / `AccessDeniedHandler` (403), reactive `ServerAuthenticationEntryPoint` / `ServerAccessDeniedHandler`; bean presence is the switch (no new properties), each independent, applied to BOTH the bearer-token path and the exception-translation path; no bean → RFC 6750 defaults unchanged.
**Config:** prefix `opentmf.security`: `jwk-set-uri` (required), `user-claim` (default `sub`), `authorities-claim` (default `roles`), `secure-endpoints[]`, `whitelist[]` (any method), `blacklist[]`, `other-endpoints` (default `DENY`); `jwks.cache-ttl` (5m), `jwks.outage-ttl` (24h), `jwks.refresh-interval` (30s), `jwks.connect-timeout` / `jwks.read-timeout` (JVM defaults, then 30s), `jwks.on-startup-failure` (`warn` default: boot and serve 503 for that issuer until a refresh succeeds; `fail`: stop when NO issuer's keys loaded), `jwks.proxy` / `issuers[].proxy` (`host:port`), `jwks.readiness` (false — 3.3.0 health contributor + readiness-group membership; `management.health.jwks.enabled` also switches it off). Separate `opentmf.security.management.*` mirror for the management port (its `whitelist` defaults to actuator health/info; `other-endpoints` defaults `AUTHENTICATED`). Autoconfigs `ServletSecurityAutoConfiguration`, `ServletJwtAutoConfiguration` (+ reactive).
**Snippet:**
```yaml
opentmf.security:
...
```
**Gotchas:** ⚠ **3.2.0 upgrade — `HTTPS_PROXY` is now honoured for the JWK fetch:** a pod that exports `HTTPS_PROXY` without the identity provider's host in `NO_PROXY` fetched the keys DIRECTLY before (Java ignores the env) and goes through the proxy from 3.2.0 — the only deliberate change for a reachable issuer; set `NO_PROXY` or `issuers[].proxy`. With the default `on-startup-failure: warn` a service boots even when its issuer is unreachable and answers 503 for that issuer's bearer tokens until a refresh succeeds — health stays UP **unless you opt in to `jwks.readiness: true` (3.3.0)**, which makes the pod NotReady for exactly that case (and only that case: a stale-but-serving cache stays UP); without the opt-in, alert on the 503 problem type or the `opentmf_security_jwks_fetch_failures_total` meter. Remove any interim filter of your own that rendered 503 for this case, or two responders compete. 3.2.1/3.2.2 (servlet only): the 503's `Retry-After` is now on the wire exactly once whoever renders it — a mapper that rebuilds the `ResponseEntity` without headers, Spring's default resolver, or the bare fallback (3.2.0 lost it through app resolvers; 3.2.1 doubled it under the default resolver). ⚠ **3.1.0 upgrade:** a path with **no handler answers 404 to everyone, token or not** — the pre-3.1.0 non-disclosure answer (401 anonymous / 403 authenticated for a path outside the rules) is gone, so any contract test, WAF rule or probe that pinned 401/403 on an unknown path breaks; a path variable carrying a `/` that falls off the route table is now a 404, not a 403. The **405 carries no `Allow` header** and applies to unknown method names too (`PROPFIND`/`BREW` were a firewall 400 with Boot JSON; on a whitelisted path a servlet 501) — tests pinning `Allow`, or 401 for an anonymous unimplemented-method request, must be rewritten. **`unmatched-method-response` (3.0.0) now FAILS STARTUP if still set** (main or management section; `RetiredPropertyGuard` names it) — remove it before deploying. Blacklisted paths no longer answer a uniform 403: unmapped → 404, unimplemented method → 405, only an existing path+method reaches the rule. The error body is the application's own resolver output; without a `@ControllerAdvice`/`ProblemDetail` renderer the service gets Spring's default resolver → the container's error page, i.e. Boot's default error JSON in a Boot app (render `NoHandlerFoundException` / `HttpRequestMethodNotSupportedException` in your advice if you want your own body). Management port: an unexposed actuator path is 404 and an unserved method 405 — alerts keyed on 401/403 from that port need 404/405 added. A static-resource handler (Boot's `/**`) claims a path only when the resource resolves and implements `GET`/`HEAD` only; a request dispatched to a non-MVC servlet (H2 console, JAX-RS) bypasses the matrix; a consumer's own firewall bean takes precedence over the library's any-method-name firewall. `other-endpoints` (`DENY` on the main port, `AUTHENTICATED` on the management port — asymmetric on purpose) still governs **mapped** paths outside the rules only. Eval order: blacklist → whitelist → allowed-endpoints → secure-endpoints → other-endpoints. Management block applies only when `management.server.port` ≠ `server.port` — **and on servlet it actually works only from 2.2.0** (in 2.1.x Boot exposed the parent filter chain in the management child context, so main-port rules governed the management port; reactive was never affected). Unknown props fail fast. Custom 401/403 beans (2.2.0): apply to the MAIN port only; multiple candidate beans of one type → default kept + WARN naming candidates, disambiguate with `@Primary`; `@PreAuthorize` denials still reach `@RestControllerAdvice` as before. Behavior matrix surprises: a present-but-invalid token on a `permitAll` URL still → 401; an anonymous request on a `denyAll`/blacklisted URL → **401, not 403** (anonymous denials translate to authentication failures). Reactive trap: re-emitting the exception via `Mono.error` into Boot's `ErrorWebExceptionHandler` yields 500, not 401/403 — render the body in the bean (shared-renderer pattern) instead. **Multi-issuer (2.3.0):** configure EXACTLY ONE of `jwk-set-uri` or `issuers` — both, or neither, fails startup by design (as do duplicate `issuer`/`name` values). There is **no fallback issuer**: a token whose `iss` matches nothing, or that carries no `iss`, is 401 — deliberate. Single-issuer mode still does NOT check `iss`, so adding `issuers` is what turns issuer validation on. ⚠ **Set `audiences` when the provider mints tokens for many apps from one tenant (Entra especially)** — otherwise a token issued to a *different application of the same tenant* passes issuer validation. Entra field traps (README): tenant- and version-specific issuer values, prefer **App Roles** over group claims (which truncate), and `oid` is the stable principal while `preferred_username`/`upn` are readable but mutable. Entries inherit top-level `user-claim`/`fallback-user-claims`/`authorities-claim` unless they override — normalize onto ONE role vocabulary so `secure-endpoints` never forks per provider. ⚠ **3.0.0 upgrade:** the `GET`→`HEAD` coverage **tightens** as well as loosens — a `HEAD` probe used to fall through to `other-endpoints` (served anonymously under `allow`), and now carries the `GET` rule's roles, so a load balancer or uptime check `HEAD`ing a `secure-endpoints` path gets 401/403 unless it presents those roles or the path is whitelisted; the management section's `other-endpoints: authenticated` default makes this bite there by default. There is no opt-out. Delete every `method: HEAD` entry (a `GET` rule covers it; a duplicate `HEAD` rule never worked anyway — Spring Security applies the first registered match) and every `method: OPTIONS` entry (an `OPTIONS` rule cannot make a browser pre-flight succeed — that needs a `CorsConfigurationSource`); both now fail startup, as does a non-upper-case value like `get` (which was silently dead before, and relaxed binding would have brought it to life on upgrade). A method the code implements but the rules withhold is still `403` (never 405); a mapped path with an unimplemented method is 405 for anonymous callers too from 3.1.0 (3.0.0 answered 401 there, and 405 + `Allow` only once authenticated). Every `403` still reaches a consumer-supplied `AccessDeniedHandler`; only the plain-`OPTIONS` answer bypasses it.
...
```
**Gotchas:** `append` is propagation **`MANDATORY`** — calling it outside a transaction fails fast, by design (a dual write cannot compile into existence). The payload is serialized **at write time**: the event is a fact frozen at commit, never re-read later. Destination routing is by shape — anything that is not an `http(s)://` URL is a Kafka topic. Delivery is at-least-once; dedupe on `x-idempotency-key` (`<spring.application.name>:outbox:<id>`). At `max-attempts` a row **parks** — excluded from claims, never auto-pruned, unparking is a deliberate ops action. The `/ops` endpoints ship **no security of their own** — the consumer must gate them as admin-class (payloads and `last_error` travel there). `tmf630-toolkit-all` is optional: without it the `/ops` controller is not registered at all (the `OutboxMaintenanceService` API is unaffected). Kafka/web deps are optional too — ⚠ **1.2.1 upgrade:** before 1.2.1 that was true only on paper for Kafka: a consumer without `spring-kafka` failed at context start with `NoClassDefFoundError: org/springframework/kafka/core/KafkaTemplate` (the auto-configuration named the type in a bean-method signature); from 1.2.1 a Kafka-less consumer starts with the HTTP publisher only, a web-less one with the Kafka publisher only, and a workaround that added `spring-kafka` + excluded `KafkaAutoConfiguration` can be dropped. The nested publisher configurations are deliberately not `@Configuration`, so a scan root covering `org.opentmf.outbox` does not register them out of order. The ArchUnit seal rule needs ArchUnit ≥ 1.5.0 on Java 25 bytecode — older ASM parses nothing and the rule silently checks **nothing**.
...
**Key API (endpoints):** engine REST + Cockpit/Tasklist/Admin web apps, as with opentmf-camunda7; actuator on its own management port. Since **1.2.0: INCIDENT OPERATIONS** under `/engine-rest/extensions/incident*` — `GET .../incident/groups?rootProcessDefinitionKey=…` reports active incidents across a root BPMN **and every BPMN it calls, at any depth and across all deployed versions**, grouped by called BPMN × task × incident type with counts, first/last occurrence and a sample message; `POST .../incident/retry` takes a group `selector` from that report verbatim and bulk-retries it through the engine's set-retries batch API in chunks (`CADENZAFLOW_INCIDENTS_RETRY_CHUNK_SIZE`, default 20000); `GET .../incident` lists individual incidents with the full TMF-630 paging contract (`offset`/`limit`, `X-Total-Count`/`Content-Range`, `Link`, 200/206/416, allowlisted `sort`, `fields=`). Also since 1.2.0, `docs/openapi.yaml` publishes the whole surface — the embedded engine's own generated spec rebased under `/engine-rest`, unioned with this service's additions and the bearer-JWT security model; it is generated by `ci/merge-openapi.py` and regenerated, never hand-edited (author changes in `ci/openapi-service.yaml`).
**Config:** env vars on the image; RBAC via `opentmf.security.*`, Keycloak via the identity plugin properties. Since **1.3.0: STANDALONE-ONLY SECURITY DEFAULTS** — the image's own `opentmf.security` block (issuer, `user-claim`, both ACLs) and its role-bearing actuator settings apply only under the `standalone` profile, which activates itself (`spring.profiles.default: standalone`) when no profile is set; activate any profile and the deployment owns `opentmf.security` entirely (`jwk-set-uri` or `issuers` required).
**Snippet:** N/A — run the published image, e.g. `docker run ghcr.io/opentmf/opentmf-cadenzaflow:1.3.0`.
**Gotchas:** Despite `jar` packaging it is a **service**, not a library — the fat jar is produced only under the `repackage`/`docker` profiles, so the artifact on Central is the plain jar. Three image flavours: plain, `-aws` (AWS JDBC wrapper for IAM auth to RDS/Aurora, STS for IRSA, MSK IAM), and `-azure` (Azure Identity for passwordless PostgreSQL) — pick the one matching your cloud, they differ in dependencies. Images are cosign-signed; releases 1.1.0 and 1.1.3 were rebuilt via `workflow_dispatch` and so carry a branch-anchored rather than tag-anchored signing identity, which a tag-anchored verification policy will (correctly) reject. ⚠ **1.3.0 upgrade — set any Spring profile and you must bring your own `opentmf.security`, or the service does not boot.** With neither `opentmf.security.jwk-set-uri` nor `opentmf.security.issuers` supplied, openid-rbac-security refuses to start — a boot-time failure no render-time gate can see, so bring up one instance before trusting a green pipeline. Before 1.3.0 a deployment that mounted no block silently inherited the image's ACL (role names meaning nothing to it), which left `/actuator`, `/actuator/metrics` and `/actuator/loggers` answering 200 **anonymously** on the management port. A bare `docker run` keeps its defaults, except that a tokenless `POST /actuator/loggers/<logger>` (which used to answer 204 and change the level) is closed: reading a level needs a valid token, changing one needs `admin`. ⚠ **Anything on 1.2.1 or older is running blind: the root logger had an empty appender list, so the service wrote NO application logs at all** while starting, serving traffic and reporting healthy. Every release 1.0.0–1.2.1 shipped a `logback-spring.xml` selecting its appender with `<if condition='isDefined("KUBERNETES_SERVICE_HOST")'>`, and Logback 1.5's `IfModelHandler` ignores the `condition` **attribute** — so neither branch ever ran. Not the usual missing-janino case (janino was on the classpath); the only trace is two stderr status lines in the JVM's startup noise. Fixed in **1.2.2** by plain substitution, `<appender-ref ref="${LOGGING_APPENDER:-CONSOLE}"/>`, both images setting `LOGGING_APPENDER=JSON`. A deployment that mounted its own file via `LOGGING_CONFIG` was unaffected and that mount stays valid. The lost events are unrecoverable — they were never written. ⚠ **1.2.1 is the security floor:** it overrides the Boot 4.1.1 BOM to embedded Tomcat 11.0.25, closing three CRITICAL `tomcat-embed-core` findings including **CVE-2026-65182** (security-constraint bypass). The override is explicit `dependencyManagement` entries declared **before** the BOM import — a `tomcat.version` property alone is silently ignored under a BOM import — and is marked for removal once a Boot GA manages 11.0.25. ⚠ **Log masking only became sound in 1.2.3:** value rules are matched against URL-encoded request lines too, where the separator is `%20`/`%2520`/`+` rather than a space. Before 1.2.2 `Basic` and opaque bearer credentials leaked **in full** (JWTs leaked only their header segment, by luck), and before 1.2.3 an encoded `+` exposed MSISDNs and card numbers verbatim, because every value rule opened with a `\b` that cannot fire after a percent-escape's trailing word character. Field-name path masks (`password`, `authorization`, …) were never affected, so structured and MDC credentials were never exposed by this.
